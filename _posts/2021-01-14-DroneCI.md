...
Apple Macs Developer Tools depends on Xcode, with an array of possible Xcode version numbers from 6 to 12, and counting.  Catalina only runs 10 and above. High Sierra will run 7-9. None of these will operating without accepting the license agreement, however the license agreement might break if switching to a newer version of Xcode. Some clang tests will not run if Apple's "Command Line Tools" are present, they should directly access Xcode without those tools - yet the package manager "brew" needed "Command Line Tools" or it couldn't install packages.  Drone will not start on a Mac during reboot, unless the Drone user has a window console sessions running too. So, Apple is not an ideal command-line-only remote server environment.  
